...
def plot_best_with_params(results):
    ...
    plt.tight_layout()
    plt.savefig("mejor_modelo_params.png")
    plt.show()
...
def plot_all_combinations(results):
    ...
    # Imprimiremos en consola el "diccionario" de puntos para que sepas qué es cada uno
    print(f"{'Modelo':<15} | {'Accuracy':<10} | {'Parámetros'}")
    print("-" * 60)

    for name, info in results.items():
        # Combinamos scores con sus respectivos parámetros
    ...
    # FIX WARNING: Asignamos 'Modelo' a 'hue' y legend=False
